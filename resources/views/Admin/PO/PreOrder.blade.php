...
@section('contents')
    <div class="container mt-5">
        ...
        <div class="modal fade" id="preOrderModal" tabindex="-1" aria-labelledby="preOrderModalLabel"
            aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="PreOrderForm" action="javascript:void(0)" method="post">
                            @csrf
                            @foreach ($item as $items)
                                <input type="hidden" name="ids[]" value="{{ $items->id }}">
                            @endforeach
                            <!-- Customer Input -->
                            <div class="mb-3">
    ...
    <script>
        ...
        $('.delete-button').on('click', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('DeletePOItem', ['id' => ':id']) }}".replace(
                            ':id', itemId),
                        type: 'POST',
                        data: {
                            "_token": "{{ csrf_token() }}"
        ...
        // Handle form submission for processing pre-orders
        $('#PreOrderForm').on('submit', function(event) {
            event.preventDefault();

            // Remove the Rupiah format before sending
            let cashValue = $('#cash').val()?.replace(/[^\d]/g, '');
            if (cashValue) $('#cash').val(cashValue);

            let formData = new FormData(this);

            $.ajax({
                url: "{{ route('MakePreOrder') }}",
                data: formData,
                type: 'POST',
                contentType: false,
                processData: false,
                success: function(result) {
                    Swal.fire({
                        icon: 'success',
                        title: 'Pre-Order Processed Successfully',
                        showConfirmButton: false,
                        timer: 2000,
                        timerProgressBar: true,
                    });
                ...
                error: function(xhr, status, error) {
                    Swal.fire({
                        icon: 'error',
                        title: 'Failed to Process Pre-Order',
                        text: 'There was an error while processing the pre-order.',
                        showConfirmButton: true
                    });
                }
            });
